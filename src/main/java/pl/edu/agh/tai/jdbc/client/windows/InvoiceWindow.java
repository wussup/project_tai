...
public class InvoiceWindow extends Window{
	...
	private void addButtonListeners(){
		...
		save.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			@Override
			public void componentSelected(ButtonEvent ce) {
				if (loginCombo.getSimpleValue() != null && name.getValue() != null && value.getValue() != null){
					...
					greetingService.uploadFile(fileName, content, userLogin, new AsyncCallback<Boolean>() {
						...
						public void onSuccess(Boolean result) {
							if (result == true){
								MessageBox.info("Info", "File " + fileName + " uploaded properly", null);
								hide();
							} else {
								MessageBox.alert("Error", "Error during uploading file to Dropbox server", null);
							}
						}
					});
				} else {
					name.isValid();
					value.isValid();
					MessageBox.alert("Warning", "Fill all required field", null);
				}
